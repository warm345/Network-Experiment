...
{
    int base; //基地址
    int nextseqnum; //基地址+发送数据包个数
    int window_size; //窗口大小
    struct pkt packet_buffer[BUFSIZE];  //缓冲区缓存发送的包
}A;

struct receiver
{
    int expected_seqnum;  //预期序号
}B;
...
void A_output(struct msg message)
{
    ...
    // printf(" from A_output: buffer the message(seq = %d):", A.nextseqnum);
    ...
    send_packet.seqnum = A.nextseqnum;
    //计算校验和
    send_packet.checksum = CHECKSUM(&send_packet);

    if(A.nextseqnum < A.base + A.window_size)
    {
        PACKET_INIT(&A.packet_buffer[A.nextseqnum % BUFSIZE]);
        //如果还可以发送，就将其存储起来，然后发送
        A.packet_buffer[A.nextseqnum % BUFSIZE] = send_packet;
...
void A_input(struct pkt packet)
{
    ...
    if(packet.acknum==NAK)
    {
        ...
        tolayer3(0,A.packet_buffer[packet.seqnum%BUFSIZE]);
        return;
    }
    //表明发送成功，base+1
...
{
    starttimer(0, 25.0);
    //没有收到ACK，对base到nextseqnum部分包进行重传
    for(int i=A.base; i<A.nextseqnum; i++)
    {
        tolayer3(0, A.packet_buffer[i%BUFSIZE]);
    }
...
void B_input(struct pkt packet)
{
    ...
    //如果出现乱码或发生乱序，向A发送NAK，重传
    if(packet.checksum != CHECKSUM(&packet)||packet.seqnum != B.expected_seqnum)
    {
        reback_packet.seqnum=B.expected_seqnum;
